...
export class BaileysStartupService extends ChannelStartupService {
        ...
        for (const received of messages) {
          try {
            if (
              ...
              try {
                const resendId = await this.client.requestPlaceholderResend(received.key);
                this.logger.warn(
                  `Message decryption/placeholder issue; requested resend. key=${received.key?.id}, resendId=${resendId}`,
                );
              } catch (error) {
                this.logger.error({
                  local: 'messages.upsert.requestPlaceholderResend',
            ...
            const originalRemoteJid = messageRaw.key?.remoteJid;
            ...
            const sendMessageWebhook = () => {
              void this.sendDataWebhook(Events.MESSAGES_UPSERT, createWebhookMessage()).catch((error) => {
                this.logger.error({
                  local: 'messages.upsert.sendDataWebhook',
                  message: error?.message,
                  stack: error?.stack,
                  keyId: received.key?.id,
                });
            ...
            // Meta Ads messages need to leave before database/media/chatbot processing can abort the handler.
            if (isMetaAdsMessage) {
              sendMessageWebhook();
            }
  ...
  private normalizePhoneNumberFromJid(jid?: string | null): string | null {
    if (!jid || !isPnUser(jid)) {
      return null;
    }
  ...
  private getCampaignMetadata(contextInfo?: any): Record<string, any> | undefined {
    if (!contextInfo) {
      return undefined;
    }
    ...
    const identifiers = {
      ...
      ref: externalAdReply?.ref,
      clientCampaignId: contextInfo.smbClientCampaignId,
      serverCampaignId: contextInfo.smbServerCampaignId,
    };

    const isMetaAds =
      contextInfo.conversionSource === 'FB_Ads' ||
      contextInfo.entryPointConversionSource === 'ctwa_ad' ||
      Object.values(identifiers).some((value) => value !== undefined && value !== null) ||
      contextInfo.ctwaSignals !== undefined ||
      contextInfo.ctwaPayload !== undefined;
    ...
    return {
      ...
      identifiers,
      ctwaSignals: contextInfo.ctwaSignals,
  ...
  private async resolvePhoneNumber(key: ExtendedIMessageKey): Promise<string | null> {
    const directCandidates = [key.remoteJidAlt, key.participantAlt, key.remoteJid, key.participant];

    for (const candidate of directCandidates) {
      const phoneNumber = this.normalizePhoneNumberFromJid(candidate);
      if (phoneNumber) {
        return phoneNumber;
      }
    }

    const lidCandidates = [key.remoteJid, key.participant, key.remoteJidAlt, key.participantAlt].filter(
      (candidate): candidate is string => !!candidate?.includes('@lid'),
    );

    for (const lid of lidCandidates) {
      try {
        const mappedJid = await this.client.signalRepository.lidMapping.getPNForLID(lid);
        const phoneNumber = this.normalizePhoneNumberFromJid(mappedJid);
        if (phoneNumber) {
          return phoneNumber;
        }
      } catch (error) {
        this.logger.debug({
          local: 'messages.upsert.resolvePhoneNumber',
          message: error?.message,
          lid,
        });
      }
    ...
    try {
      const cachedMappings = await getOnWhatsappCache(lidCandidates);
      for (const mapping of cachedMappings) {
        const phoneNumber = this.normalizePhoneNumberFromJid(`${mapping.number}@s.whatsapp.net`);
        if (phoneNumber) {
          return phoneNumber;
        }
      }
      ...
      this.logger.debug({
        local: 'messages.upsert.resolvePhoneNumber.cache',
        message: error?.message,
      });
    }
