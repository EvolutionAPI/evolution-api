...
export class RabbitmqController extends EventController implements EventControllerInterface {
  ...
  private async connect(): Promise<void> {
    ...
    try {
      ...
      const heartbeat = configService.get<Rabbitmq>('RABBITMQ').HEARTBEAT;
  ...
  public async emit({
      ...
      if (Array.isArray(rabbitmqLocal) && rabbitmqLocal.includes(we)) {
        ...
        while (retry < 3) {
          try {
            ...
            if (retry >= 3) {
              this.logger.error('Max retry attempts reached for RabbitMQ publish');
              if (!this.amqpChannel) {
                this.scheduleReconnect();
              }
            } else {
    ...
    if (rabbitmqGlobal && rabbitmqEvents[we] && this.amqpChannel) {
      ...
      while (retry < 3) {
        try {
          if (!this.amqpChannel) {
            this.logger.warn('RabbitMQ channel not available, skipping global message');
            break;
          }
          ...
          if (retry >= 3) {
            this.logger.error('Max retry attempts reached for RabbitMQ global publish');
            if (!this.amqpChannel) {
              this.scheduleReconnect();
            }
          } else {
