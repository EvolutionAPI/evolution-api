...
import { ProxyRaw, SettingsRaw } from '../models';
...
export class WABaileysService extends WAStartupService {
  constructor(
    ...
    this.logger.verbose('WABaileysService initialized');
    this.instance.qrcode = { count: 0 };
  }

  public client: WASocket;
  protected readonly msgRetryCounterCache: CacheStore = new NodeCache();
  protected readonly userDevicesCache: CacheStore = new NodeCache();
  protected logBaileys = this.configService.get<Log>('LOG').BAILEYS;
  ...
  public async setSettings(data: SettingsRaw) {
    await super.setSettings(data);
    this.client?.ws?.close();
  }

  public async setProxy(data: ProxyRaw, reload = true) {
    await super.setProxy(data);
    if (reload) {
      this.reloadConnection();
    }
  }

  protected async connectionUpdate({ qr, connection, lastDisconnect }: Partial<ConnectionState>) {
  ...
  protected async getMessage(key: proto.IMessageKey, full = false) {
  ...
  protected readonly contactHandle = {
  ...
  protected readonly messageHandle = {
  ...
  protected readonly groupHandler = {
  ...
  protected formatMXOrARNumber(jid: string): string {
  ...
  protected formatBRNumber(jid: string) {
    ...
    try {
      ...
      if (number) {
        ...
        return {
          ...
          website: business?.websites?.shift(),
        };
      } else {
  ...
  protected async sendMessageWithTyping<T = proto.IMessage>(
    number: string,
    message: T,
    options?: Options,
    isChatwoot = false,
  ...
  protected async formatStatusMessage(status: StatusMessage) {
  ...
  protected async convertToWebP(image: string, number: string) {
    try {
