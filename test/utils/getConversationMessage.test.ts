...
describe('getConversationMessage', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (configService.get as jest.Mock).mockReturnValue({ ENABLE: false, SAVE_VIDEO: false });
  });
  ...
  it('should format an image message using message ID when S3 is disabled', () => {
    ...
    expect(getConversationMessage(msg)).toBe('imageMessage|MSG123|Beautiful picture');
  });

  it('should format a video message using mediaUrl when S3 is enabled and SAVE_VIDEO is true', () => {
    (configService.get as jest.Mock).mockReturnValue({ ENABLE: true, SAVE_VIDEO: true });
    
    const msg = {
      key: { id: 'MSG123' },
      message: { mediaUrl: 'https://s3.aws.com/video.mp4', videoMessage: { caption: 'Funny video' } }
    };
    expect(getConversationMessage(msg)).toBe('videoMessage|https://s3.aws.com/video.mp4|Funny video');
  });
  ...
  it('should handle externalAdReply body and append it to the message', () => {
    ...
    expect(getConversationMessage(msg)).toBe('Check out this ad\nexternalAdReplyBody|Ad Content Here');
  });
  ...
  it('should handle externalAdReply body when it is the only content', () => {
    ...
    expect(getConversationMessage(msg)).toBe('externalAdReplyBody|Only Ad Content');
  });

  it('should return undefined when message format is unknown', () => {
    const msg = { message: { unknownFormat: '???' } };
    expect(getConversationMessage(msg)).toBeUndefined();
  });
});
