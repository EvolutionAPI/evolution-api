...
from app.services.ticket_repository import TicketCommentRepository
...
async def add_comment(ticket_id: int, data: TicketCommentCreate, db: Session = Depends(get_db)):
...
async def delete_comment(ticket_id: int, comment_id: int, db: Session = Depends(get_db)):
    repo = TicketCommentRepository(db)
    comment = repo.get(comment_id)
    if not comment or comment.ticket_id != ticket_id:
        raise HTTPException(status_code=404, detail='Comment not found')
